    ...
    def Connect_Arduino(self):
        ...
        try:
            ...
            if msg.decode('utf-8').rstrip() == 'Found sensor':
                ...
                status.Arduino_connection = True    # Change global variable
                status.Arduino_connection_real = True
            elif msg.decode('utf-8').rstrip() == "Checking for AS7341 color sensor...":
                msg_popup = CTkMessagebox(title="Found sensor", message="Arduino is loaded with code for Colour sensor AS7341!", 
                                        icon="check", option_1="OK")
                status.Arduino_connection = True    # Change global variable
                status.Arduino_connection_real = True
            else: 
                msg_popup = CTkMessagebox(title="Sensor Error", message="Colour sensor cannot be detected!\n Received COM data: \"" + msg.decode('utf-8').rstrip() + "\"", 
